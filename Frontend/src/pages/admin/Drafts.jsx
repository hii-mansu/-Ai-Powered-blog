    ...
    <div className="m-7">
      <div className="w-full flex justify-between items-center">
        <div className="flex flex-row gap-2 items-center pl-4">
          ...
          <span className="text-violet-600 text-xl font-semibold">Drafts</span>
        </div>
        <Link
          to="/admin"
          className="px-2.5 py-1 rounded bg-violet-600 text-white"
        >
          Write Blog
        </Link>
      </div>
      <div className="mt-10">
        <div className="overflow-x-auto bg-white rounded-xl border border-violet-300">
          <div className="bg-violet-50 p-4">
            <h5 className="flex flex-row gap-2 items-center text-violet-600">
              <span className="border border-violet-200 text-xs px-2 py-0.5 rounded-lg">
                99
              </span>{" "}
              Drafts found
            </h5>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {isLoading ? <p>Loading......</p> :(data.blogs
                .filter((blog) => blog.isLive != true)
                .map((item, index) => (
                  <tr
                    key={item.id}
                  ...
                  >
                    <td className="p-3">
                      <span className="py-0.5 px-1 rounded-full border-2 bg-violet-500 border-violet-400 text-xs text-white font-bold">
                        {index + 1}
                      </span>
                    </td>

                    <td className="p-3 font-normal text-gray-500">
                      {item.title}
                    </td>
                    ...
                    <td className="p-3 flex gap-1 flex-wrap">
                      ...
                      <span className="px-2 py-0.5 text-xs rounded text-gray-500">
                        + more
                      </span>
                    </td>
